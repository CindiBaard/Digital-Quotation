...
with st.sidebar:
    ...
    u_layout = st.number_input("Layout Design and Finished artwork", min_value=0, value=0)
    u_barcode = st.number_input("Generate Barcode", min_value=0, value=0)
    u_photo = st.number_input("Photo manipulation and Deep-etching", min_value=0, value=0)
    u_colour = st.number_input("Colour retouching", min_value=0, value=0)
    u_foil = st.number_input("Foil block", min_value=0, value=0)
...
with tab2:
    ...
    try:
        ...
        if all_data:
            ...
            quote_list = df_db["Quote No."].tolist()
            ...
            with col_del2:
                ...
                if st.button("Delete Permanently"):
                    try:
                        ...
                        st.success(f"Quote {to_delete} deleted!")
                        st.rerun()
                    except:
